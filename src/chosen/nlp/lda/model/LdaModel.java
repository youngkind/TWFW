...
*/
    ...
    */
	    for (int v = 0; v < V; v++) {
	    	...
	    	for (int k = 0; k < K; k++){
	    		...
	    		plv.PMI[k][v] = p_kt*Math.log(p_kt);
	    		if(Double.isNaN(plv.PMI[k][v]))
	    			plv.PMI[k][v] = 0;
	    	}
	    	double PMI_Sum = 0;
	    	...
	    	plv.PMI_v_Sum[v] = PMI_Sum/Math.log(K-1)+1;
	      }
  }
  ...
  public void Save(int iters) throws IOException {
    ...
    HashSet<String> financeWordSet = new HashSet<String>();
    String[] flagList = {"j","Ng","n","nt","vg","vd"};
    String[] keyWordList = {"降息","经济","市场","深港","利率"};
    for (int i = 0; i < K; i++) {
      List<Integer> tWordsIndexArray1 = new ArrayList<Integer>();
      for (int j = 0; j < V; j++) {
        tWordsIndexArray1.add(new Integer(j));
      }
      // Construct index of phi[i] sorted by its value
      // which means that index = tWordsIndexArray.get(0) , phi[i][index] is the
      // biggest value
      Collections.sort(tWordsIndexArray1, new LdaModel.TwordsComparable(phi[i]));
      for(int k = 0; k < keyWordList.length; k++){
    	  if(trainSet.indexToTermMap.get(tWordsIndexArray1.get(0)).contains(keyWordList[k])){
    		  for (int t = 0; t < 100; t++) {
    	    	  String w = trainSet.indexToTermMap.get(tWordsIndexArray1.get(t));
    	    	  String w_flag = w.split("_")[1];
    	    	  for(int f=0; f < flagList.length; f++){
    	    		  if(w_flag.equals(flagList[f])){
    	    			  financeWordSet.add(w);
    	    			  break;
    	    		  }
    	    	  }
    	      }
    ...
    for(String f_word:financeWordSet){
    	writer.write(f_word+"\n");
    }
    writer.close();
    */
  ...
  public int labelSample(int m, int n) {
    ...
    return labelGibbsSampler(m, word);
    //return formerTopic;
  }
